...
void save_tq(uint8_t * mac, int tq) {
    char mac_str[18];
    mac_to_str(mac_str, mac);
    printf("%s %d\n", mac_str, tq);

    char file_name[25];
    sprintf(file_name, "%s", mac_str);
    FILE * out;
...
static int tq_callback(struct nl_msg* msg, void* arg __maybe_unused) {
    ...
    if(nla_parse(attrs, 60, genlmsg_attrdata(ghdr, 0), genlmsg_len(ghdr), batadv_netlink_policy)) {
        fputs("Received invalid data from kernel.\n", stderr);
		exit(5);
    }
...
int main(int argc, char ** argv) {
	...
	while ((optchar = getopt(argc, argv, "+w:cr:")) != -1) {
        ...
        case 'r':
			...
			if (!sscanf(optarg, "%f", &repetitions)) {
				fprintf(stderr, "Error - provided argument of '-%c' is not a number\n", optchar);
				return 21;
			}
		default:
	...
	nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, tq_callback, NULL);
    ...
    while(repetitions == -1 || i < repetitions) {
        ...
        if (!msg)
            return 4;
        genlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, batadv_family, 0, NLM_F_DUMP, 8, 1);
